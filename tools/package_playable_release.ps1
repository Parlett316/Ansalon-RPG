...
.DESCRIPTION
    ...
    Each run bumps a build counter in tools/playable_release_version.txt
    (checked into git so the increment shows up in `git diff`) and stamps
    it into the zip name (dist/AnsalonRPG-Playable-v<N>.zip) and a
    VERSION.txt dropped inside the package.
